...
"""
엔드투엔드:
- h5에서 step 개수/액션 요점 수치 추출
- 비디오에서 step→대표 프레임 추출
- 라벨 CSV에서 자연어 설명/태그 로드
- (선택) 메타 JSON에서 current_task 로드
- SigLIP으로 텍스트/이미지 임베딩 생성(차원 통일)
- (기본) 텍스트와 이미지 임베딩 평균 → ES JSONL 저장
"""
import os, json
import pandas as pd
import numpy as np
...
def main():
    os.makedirs(os.path.dirname(PIPE.out_jsonl) or ".", exist_ok=True)
    ...
    print(f"[INFO] embed_dim={emb.embed_dim}, model={MODEL.model_name}")

    out_path = PIPE.out_jsonl
    fout = open(out_path, "w", encoding="utf-8")

    labels_by_idx = {int(r["idx"]): r for _, r in labels.iterrows()}

    step_indices = list(range(0, n_steps_h5, PIPE.step_stride))
    for idx in tqdm(step_indices, desc="Embedding steps"):
    ...
    fout.close()
    print(f"[DONE] ES 적재용 JSONL 저장: {out_path}")
